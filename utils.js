...
function loadFonts() {
  ["black", "bold", "light", "medium", "regular", "thin"].map((item) => {
    ...
    canvas.registerFont(`./resources/fonts/roboto-${item}.ttf`, {
      family: family,
    });
    debug.sendInfo(`Registering ${family}`, -1);
  });
  debug.sendInfo("Registered all fonts");
}
...
function discordException(client, err, message = null, command = null) {
  if (message != null) {
    var embed = new Discord.MessageEmbed()
      ...
      .setTitle("Error")
      .setDescription(
        "Something went wrong trying to execute the command. The devs have been notified about this issue. Please try again later."
      )
      .setTimestamp()
      .setFooter(
...
function getPrefixes() {
  if (production) {
    return config.prefixes.production;
  }
  return config.prefixes.beta;
}
...
function updateActivity(client, prefixes) {
  ...
  setInterval(() => {
    ...
    client.user.setActivity(
      `${client.guilds.cache.size} Guilds [ ${prefixes[prefixIndex]}help ]`,
      {
        type: "WATCHING",
      }
    );
...
function updateIntervals(client, dontGiveXPto) {
  ...
  setInterval(() => {
    for (var guild of client.guilds.cache.array()) {
      let c = guild.channels.cache.find((chnl) =>
        chnl.name.includes("Member Count:")
      ); //chnl.name == `Member Count: ${usrCount-1}`)
      if (c) {
        var usrCount = guild.members.cache.filter((mmbr) => !mmbr.user.bot)
          .size;
